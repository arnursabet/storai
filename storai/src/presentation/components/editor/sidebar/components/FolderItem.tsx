...
interface FolderItemProps {
  ...
  onStartRenaming: (folderId: string, e: React.MouseEvent) => void;
}
...
const FolderItem: React.FC<FolderItemProps> = ({
  ...
  onStartRenaming
    ...
    const {setActiveFolder, toggleFolderExpanded} = useEditorContext();

    return (
        <div key={folder.id}>
            <div 
                className={`flex items-center p-2 rounded-md cursor-pointer ${folder.isActive ? 'bg-gray-100' : 'hover:bg-gray-50'}`}
            ...
            {folder.isRenaming ? (
                ...
                <span 
                    ...
                    onClick={(e) => onStartRenaming(folder.id, e)}
                >
                    {folder.name}
                </span>
            )}
